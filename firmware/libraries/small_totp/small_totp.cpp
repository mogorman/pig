...
uint32_t small_totp::code(uint32_t time) {
  ...
  sha1_write(Google_time_array,8);
  Big_hash = sha1_resultHmac();
...
//PRIVATE
void small_totp::sha1_initHmac(sha1nfo *s, const uint8_t* key, int keyLength) {
	uint8_t i;
	...
	for (i=0; i<BLOCK_LENGTH; i++) {
		sha1_writebyte(s->keyBuffer[i] ^ HMAC_IPAD);
	}
}

uint8_t* small_totp::sha1_resultHmac(sha1nfo *s) {
	uint8_t i;
	// Complete inner hash
...
void small_totp::sha1_init(sha1nfo *s) {
...
void small_totp::sha1_writebyte(uint8_t data) {
	s.byteCount++;
	sha1_addUncounted(data);
}

void small_totp::sha1_addUncounted(uint8_t data) {
...
uint8_t* small_totp::sha1_result(void) {
...
void small_totp::sha1_pad(void) {
	// Implement SHA-1 padding (fips180-2 §5.1.1)

	// Pad with 0x80 followed by 0x00 until the end of the block
	sha1_addUncounted(0x80);
	while (s.bufferOffset != 56) sha1_addUncounted(0x00);
	...
	sha1_addUncounted(s.byteCount >> 29); // Shifting to multiply by 8
	sha1_addUncounted(s.byteCount >> 21); // as SHA-1 supports bitstreams as well as
	sha1_addUncounted(s.byteCount >> 13); // byte.
	sha1_addUncounted(s.byteCount >> 5);
	sha1_addUncounted(s.byteCount << 3);
}

void small_totp::sha1_write(const byte *data, size_t len) {
	for (;len--;) sha1_writebyte((uint8_t) *data++);
}
...
void small_totp::sha1_hashBlock(void) {
	uint8_t i;
